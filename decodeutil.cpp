...
bool DecodeUtil::Open(AVCodecParameters *codec_param) {
  ...
  if (result != 0) {
    ...
    lock.unlock();
    char errbuf[64] = {0};
    qDebug() << "打开解码器失败" << av_make_error_string(errbuf, 64, result);
    return false;
  }
  lock.unlock();
  qDebug() << "查找到的解码器名称" << codec->name;
  //    avcodec_parameters_free(&codec_param);
  return true;
}
...
// 获取解码后数据，一次send可能需要多次receive，获取缓冲中的数据Send
// NULL，再Receive多次 每次复制一份，由调用者释放空间 av_frame_free()
AVFrame *DecodeUtil::Receive() {
  ...
  //    qDebug() << "DecodeUtil::Receive standard_pts"
  //    <<  standard_pts;
  return frame;
}
