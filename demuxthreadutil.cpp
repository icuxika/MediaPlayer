...
bool DemuxThreadUtil::Open(const char *url, VideoPlayWidget *widget) {
  ...
  if (!videoThreadUtil->Open(demuxUtil->getVideoParameters(), widget,
                             demuxUtil->getVideoParameters()->width,
                             demuxUtil->getVideoParameters()->height)) {
    result = false;
    qDebug() << "打开视频解码器和处理线程失败";
  }
  if (!audioThreadUtil->Open(demuxUtil->getAudioParameters())) {
    result = false;
    qDebug() << "打开音频解码器和处理线程失败";
  }
  lock.unlock();
  // 用来计算播放进度
...
void DemuxThreadUtil::Clear() {
  ...
  if (demuxUtil) {
    demuxUtil->Clear();
    qDebug() << "demuxUtil清理结束";
  }
  if (videoThreadUtil) {
    videoThreadUtil->Clear();
    qDebug() << "videoThreadUtil清理结束";
  }
  if (audioThreadUtil) {
    audioThreadUtil->Clear();
    qDebug() << "audioThreadUtil清理结束";
  }
  lock.unlock();
}
...
void DemuxThreadUtil::Seek(double pos) {
  ...
  qDebug() << "pauseStatus" << pauseStatus;
  lock.unlock();
  // 设置暂停
  SetPause(true);
  qDebug() << "设置暂停成功";
