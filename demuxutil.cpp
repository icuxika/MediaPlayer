...
bool DemuxUtil::Open(const char *url) {
  ...
  if (result != 0) {
    ...
    qDebug() << "Can't open the file!"
             << av_make_error_string(errbuf, 64, result);
    return false;
  }
  // 获取流信息
  ...
  qDebug() << QString::fromLocal8Bit(
      "------------------------------------------------------------");
  qDebug() << "视频信息（videoStream）" << videoStream;
  qDebug() << "视频宽度" << vStream->codecpar->width;
  qDebug() << "视频高度" << vStream->codecpar->height;
  qDebug() << "视频帧率FPS" << fraction2double(vStream->avg_frame_rate);
  qDebug() << "视频像素格式" << vStream->codecpar->format;
  qDebug() << QString::fromLocal8Bit(
      "------------------------------------------------------------");
  audioStream =
      av_find_best_stream(fmt_ctx, AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
  AVStream *aStream = fmt_ctx->streams[audioStream];
  qDebug() << "音频信息（audioStream）" << audioStream;
  qDebug() << "样本率" << aStream->codecpar->sample_rate;
  qDebug() << "音频采样格式" << aStream->codecpar->format;
  qDebug() << "声道数" << aStream->codecpar->channels;
  qDebug() << "编码格式" << aStream->codecpar->codec_id;
  qDebug() << "音频FPS" << fraction2double(aStream->avg_frame_rate);
  qDebug() << "一帧音频的大小" << aStream->codecpar->frame_size;
  qDebug() << QString::fromLocal8Bit(
      "------------------------------------------------------------");
  lock.unlock();
  return true;
...
AVPacket *DemuxUtil::Read() {
  ...
  if (result != 0) {
    ...
    av_packet_free(&pkt);
    char errbuf[64] = {0};
    qDebug() << "读取AVPacket失败" << av_make_error_string(errbuf, 64, result);
    return nullptr;
  }
  // 转换为毫秒
  pkt->pts =
      pkt->pts * (1000 * (fraction2double(
                             fmt_ctx->streams[pkt->stream_index]->time_base)));
  pkt->dts =
      pkt->dts * (1000 * (fraction2double(
                             fmt_ctx->streams[pkt->stream_index]->time_base)));
  //    qDebug() << "当前stream_index" <<
  //    pkt->stream_index << "pts" << pkt->pts;
  lock.unlock();
  return pkt;
}
...
AVCodecParameters *DemuxUtil::getVideoParameters() {
  ...
  avcodec_parameters_copy(videoParameters,
                          fmt_ctx->streams[videoStream]->codecpar);
  lock.unlock();
...
AVCodecParameters *DemuxUtil::getAudioParameters() {
  ...
  avcodec_parameters_copy(audioParameters,
                          fmt_ctx->streams[audioStream]->codecpar);
  lock.unlock();
...
bool DemuxUtil::Seek(double pos) {
  ...
  int result = av_seek_frame(fmt_ctx, videoStream, seekPos,
                             AVSEEK_FLAG_BACKWARD | AVSEEK_FLAG_FRAME);
  lock.unlock();
  if (result < 0) {
    char errbuf[64] = {0};
    qDebug() << "Seek失败" << av_make_error_string(errbuf, 64, result);
    return false;
  }
  return true;
}
