...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  this->setGeometry((rect.width() - 1280) / 2, (rect.height() - 760) / 2, 1280,
                    760);
  ...
  playOrPauseButton = new QPushButton("播放");
  ...
  connect(playOrPauseButton, &QPushButton::clicked, this,
          &MainWindow::PlayOrPause);
  connect(progressBar, &QSlider::sliderPressed, this,
          &MainWindow::progressBarPress);
  connect(progressBar, &QSlider::sliderReleased, this,
          &MainWindow::progressBarRelease);
  // 更新播放进度
  startTimer(40);
}

MainWindow::~MainWindow() { demuxThreadUtil.Close(); }

// 打开播放目标文件
void MainWindow::OpenFile() {
  QString fileUrl = QFileDialog::getOpenFileName(this, "选择视频文件");
  if (fileUrl.isEmpty()) {
    return;
  }
  ...
  if (!demuxThreadUtil.Open(fileUrl.toLocal8Bit(), videoPlayWidget)) {
    QMessageBox::information(nullptr, "ERROR", "无法打开文件");
    return;
  }
  // 启用播放暂停按钮
...
// 点击进度条（不松）
void MainWindow::progressBarPress() { isProcessBarPress = true; }
