...
bool ResampleUtil::Open(AVCodecParameters* codec_param, bool is_param_clear)
{
    ...
    if (!codec_param)
    {
        qDebug() << QString::fromLocal8Bit("codec_param不能为空");
        return false;
    }
    lock.lock();
    // 音频重采样，上下文初始化
    ...
    swr_ctx = swr_alloc_set_opts(
                  ...
                  (AVSampleFormat)codec_param->format,
    ...
    {
        char errbuf[64] = {0};
        qDebug() << QString::fromLocal8Bit("重采样初始化失败") << av_make_error_string(errbuf, 64, result);
        return false;
    }
    return true;
}
...
int ResampleUtil::Resample(AVFrame* frame, unsigned char* data)
{
    ...
    if (result < 0)
    {
        av_frame_free(&frame);
        char errbuf[64] = {0};
        qDebug() << QString::fromLocal8Bit("重采样失败") << av_make_error_string(errbuf, 64, result);
        return result;
    }
    int resample_size = result * frame->channels * av_get_bytes_per_sample((AVSampleFormat)outFormat);
    av_frame_free(&frame);
